...
public class Polls : MonoBehaviour
{
    ...
    private const float torqueAfterCut = 5.0f;
    ...
    void Start()
    {
        ...
        {
           victim.GetComponent<Rigidbody>().AddTorque(-torqueAfterCut, 0, 0, ForceMode.Impulse);
    ...
    [SerializeField] int diskIncreaseNumber = 3; //白のポールにヒットした時のディスクの増加量
    ...
    void OnTriggerExit(Collider collision)
    {
        if(collision.gameObject.tag == "Disk")
        {
            ...
            {
                gameController.DiskCount = diskIncreaseNumber;
                DisplayPopUp(diskIncreasePopUp);
            }
            else if(this.gameObject.tag == "RedPoll")
            {
                if(!gameController.GetItemUseStatus("ColorStop"))
                {
                    gameController.ColorBarPosition = colorIncreaseCount;
